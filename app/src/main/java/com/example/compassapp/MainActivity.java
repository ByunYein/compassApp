...
public class MainActivity extends AppCompatActivity {
    ...
    float[] accelValue, magneticValue;
            ...
            public void onSensorChanged(SensorEvent event) {
                ...
                switch (event.sensor.getType()){
                    case Sensor.TYPE_ACCELEROMETER:
                        accelValue=event.values.clone(); break;
                    case Sensor.TYPE_MAGNETIC_FIELD:
                        magneticValue = event.values.clone(); break;

                    default: break;
                }
                if(magneticValue!=null && accelValue!=null){
                    float[] R = new float[16];
                    float[] I = new float[16];
                    SensorManager.getRotationMatrix(R,I,accelValue,magneticValue);
